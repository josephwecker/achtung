...
all_transformations(Defs) ->
  lists:foldl(fun ast_map/2,
    Defs, [
      %fun tr_simple_lit_to_chr/1,
      %fun tr_combine_chr/1
      fun tr_lits_to_chrs/1,
      fun tr_expand_chrs/1,
      fun tr_expand_plusses/1    %% Now in 2nd form (assuming inlining has taken place)
      %fun tr_condense_pchs_seqs/1
    ]).
...
expr_map(Expr, Fun) ->
  ...
  Expr2 = Fun(Expr),
  case Expr2 of
    {pch,Attr,L} -> {pch,Attr,[expr_map(L2,Fun)||L2<-L]};
...
matches_capture(Tag,{ref,_,Tag}) -> true;
...
get_tl_inner(Defs,{ref,_,Name},Seen,Tops) ->
...
%--------------------------------------- TRANSFORMATIONS ---------------------
% Multi-level unalias
tr_inline({ref,CallAttrs,Name}=Call, TLNames, Defs) ->
...
%% Before:  a <- 'a' 'bc'
%% After:   a <- [a] 'bc'
%tr_simple_lit_to_chr({lit,Attrs,<<Chr>>}) -> {chr,Attrs,[Chr]};
%tr_simple_lit_to_chr(Other) -> Other.

%% Combine pch-charclasses into charclass. Captures no problem because we're in
%% an pch, but if any are prefixed or suffixed this transform doesn't apply.
%% Before:  a <- [a]/[b]/[c]
%% After:   a <- [abc]
%tr_combine_chr({pch,Attr,PchList})->{pch,Attr,tr_combine_chr_inner(PchList,[])};
%tr_combine_chr(Other) -> Other.
%tr_combine_chr_inner([],Acc) -> lists:reverse(Acc);
%tr_combine_chr_inner([One],Acc) -> lists:reverse([One|Acc]);
%tr_combine_chr_inner([{chr,A1,Ranges1}|[{chr,A2,Ranges2}|R]],Acc) ->
%  case {A1,A2} of
%    {[],[]} -> 
%  ((A1 == []) or (A1 == [orig])) and ((A2 == []) or (A2 == [orig])) ->
%    tr_combine_chr_inner([{chr,A1++A2,lists:usort(Ranges1++Ranges2)}|R], Acc);
%tr_combine_chr_inner([Other|R],Acc) -> tr_combine_chr_inner(R,[Other|Acc]).
...
tr_lits_to_chrs({lit,A,<<Char>>}) ->{chr,A,[Char]};
tr_lits_to_chrs({lit,A,Chars})->{seq,A,[{chr,[],[C]}||C<-binary_to_list(Chars)]};
tr_lits_to_chrs(Other)->Other.

tr_expand_chrs({chr,_,[_]}=E)-> E;
tr_expand_chrs({chr,Attrs,Ranges})->{pch,Attrs,[{chr,[],[R]}||R<-Ranges]};
tr_expand_chrs(Other)->Other.
...
%      end
%  end.
%first_fixed_i({chr,_,[R]}) -> R;
%first_fixed_i({pch,A,Exprs}) ->
%  R = common_prefix([first_fixed(E)||E<-Exprs]),
  %io:format("Common prefix for ~p: <~s>~n",
  %  [{pch,A},lists:map(fun({C1,C2})->[$[,C1,$-,C2,$]];(C3)->C3 end, R)]),
%  R;
...
% pch within seq always parenth.
% pch within pch always parenth.
...
pretty_print_expr({chr,Attr,Ranges},_Lvl,_Parent) ->
...
pretty_print_expr({ref,Attr,Name},_Lvl,_Parent) ->
  [pp_prefs(Attr),atom_to_list(Name),pp_suffs(Attr)];
pretty_print_expr({S,Attr,_},_Lvl,_Parent) when S==fin;S==eof;S==eps;S==fail ->
  [pp_prefs(Attr),atom_to_list(S),pp_suffs(Attr)];
pretty_print_expr({pch,Attr,Exprs},Lvl,Parent) ->
  Prefs = pp_prefs(Attr),
  Suffs = pp_suffs(Attr),
  Paren  = (length(Prefs++Suffs) > 0) or (Parent == seq) or (Parent == pch),
  {Begin,End} = case Paren of false->{[],[]};_->{[Prefs,$(],[$),Suffs]} end,
  case all_atomics(Exprs) of
    true -> [Begin,string:join([pretty_print_expr(E,Lvl+1,pch)||E<-Exprs],"/"),End];
    false-> [Begin,string:join([pretty_print_expr(E,Lvl+1,pch)||E<-Exprs]," / "),End]
  end;
