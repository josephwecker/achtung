...
optimize([{rule,First,_,_}|_] = AST) ->
  ...
  {_,{Defs3,TopLevels,_}} =
  lists:mapfoldl(fun tr_inline/2,
    {Defs2,dict:from_list([{E,true}||E<-Entrances]),Entrances},Entrances),
...
tr_inline(Entrance, {Defs, TLs, Seen}) ->
  % TODO: here combine scan_tls (above) and tr_inline_all (below)
  %   But separate the lit -> char to its own transformation
  {ok, expr_mapfold(fun tr_inline_inner/2,
      {Defs,TLs,Seen},Defs:fetch(Entrance))}.

tr_inline_inner({call, CallAttr, Name}=InExpr, {Defs, TLs, Seen}) ->
  case TLs:is_key(Name) of
    true -> InExpr;
    false -> case lists:member(Name,Seen) of
        true -> 
