...
'grammar'(Input, Index) ->
  p(Input, Index, 'grammar', fun(I,D) -> (p_zero_or_more(fun 'comment_block'/2))(I,D) end, fun(Node, Idx) -> transform('grammar', Node, Idx) end).

'begin'(Input, Index) ->
  p(Input, Index, 'begin', fun(I,D) -> (p_string("(*"))(I,D) end, fun(Node, Idx) -> 'begin' end).

'end'(Input, Index) ->
  p(Input, Index, 'end', fun(I,D) -> (p_string("*)"))(I,D) end, fun(Node, Idx) -> 'end' end).

'comment'(Input, Index) ->
  p(Input, Index, 'comment', fun(I,D) -> (p_choose([fun 'comment_block'/2, p_one_or_more(p_seq([p_not(fun 'begin'/2), p_not(fun 'end'/2), p_anything()]))]))(I,D) end, fun(Node, Idx) -> 
  {comment, case Node of
      [_|_] -> lists:flatten(Node);
      _ -> Node
  end} end).

'comment_block'(Input, Index) ->
  p(Input, Index, 'comment_block', fun(I,D) -> (p_seq([fun 'begin'/2, p_zero_or_more(fun 'comment'/2), fun 'end'/2]))(I,D) end, fun(Node, Idx) -> {comment_block, p2(Node)} end).
